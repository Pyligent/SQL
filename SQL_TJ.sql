...
, actor.last_name FROM sakila.actor;
...
SELECT actor.last_name AS 'last name'
, count(actor.actor_id) AS ' total number '
...
SELECT actor.last_name AS 'last name'
, count(actor.actor_id) AS ' total number at least two'
...
,staff.first_name
,staff.last_name
,sum(payment.amount) AS 'payment amount'

FROM sakila.staff

LEFT JOIN sakila.payment ON staff.staff_id  = payment.staff_id
...
SELECT count(inventory.inventory_id) AS 'number of copies'
...
WHERE (film.title LIKE 'K%' OR film.title LIKE 'Q%') AND film.language_id 
		IN  ( SELECT language.language_id FROM sakila.language
...
LEFT JOIN sakila.city ON address.city_id = city.city_id
...
SELECT film.title 
,count(film.title) AS 'Rental Number'
...
INNER JOIN sakila.inventory  ON film_category.film_id = inventory.film_id
